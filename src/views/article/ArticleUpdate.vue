      ...
      >
        <a-collapse v-model="activeKey" class="post-settings" :bordered="false">
          <a-collapse-panel header="URL" key="1">
           ...
           <a-collapse-panel header="author" key="7">
            <!-- <a-input v-model="form.author" @change="handleAuthorChange"></a-input> -->
...
export default class ArticleUpdate extends Vue {
  ...
  buildCurrentForm() {
    ...
    if (articleFileName) {
      ...
      if (currentPost) {
        ...
        this.form.author = currentPost.data.author
  ...
  // handleAuthorChange(val: string) {
  //   this.authorChanged = !!val
  // }
